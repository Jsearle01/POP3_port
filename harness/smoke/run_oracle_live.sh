...
echo "[oracle_live]   ~82 s  HE RAISES HIS ARMS, the hourglass appears, the flash"
echo "[oracle_live]   ~85 s  HIS EXIT -- he turns and WALKS OUT MIRRORED"
echo "[oracle_live]"
echo "[oracle_live] ** THE EXIT WALK IS WHAT TO WATCH (P3.98/P3.99). The port's walk-out"
echo "[oracle_live]   lurches: the drawn column moves +5,+3,+5,+1,+9,-3 through the cycle"
echo "[oracle_live]   where the entry walk is a smooth -4,-12,-3,+2,-1,-2. That is NOT a"
echo "[oracle_live]   port defect -- a mirrored image is laid one sprite-width to the LEFT"
echo "[oracle_live]   of its coordinate [HIRES.S MLayGen: LDA XCO / SEC / SBC WIDTH], the"
echo "[oracle_live]   walk cels are 3,4,5,5,4,4 bytes wide, and TRACED ON THIS MACHINE the"
echo "[oracle_live]   oracle subtracts widths of 2,5,7,11,12 on different mirrored draws."
echo "[oracle_live]   So the lurch should be HERE TOO. Watch his walk out and compare."
echo "[oracle_live]   If it looks smooth here and rough in the port, that is a finding and"
echo "[oracle_live]   the trace missed something -- tell me and I will re-open it."
echo "[oracle_live]"
echo "[oracle_live] The timings above were MEASURED, not estimated: the exit is armed off"
echo "[oracle_live]   PlayCut0's own SPEED/psandcount markers at frame 4886 (~81.5 s)."
