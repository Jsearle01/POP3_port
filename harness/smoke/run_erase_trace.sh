#!/bin/bash
# harness/smoke/run_erase_trace.sh
#
# POP P3.72 §7 — the erase record, per frame, across the princess's one move.
# See harness/tools/erase_trace.lua for what is being asked and what is already
...
DSK="build/run_erase.dmk"
MAP="build/obj/room.map"
FMAP="build/obj/flames.map"

[ -f "$MAP" ]  || { echo "[erase_trace] missing $MAP — run build.bat first";  exit 1; }
[ -f "$FMAP" ] || { echo "[erase_trace] missing $FMAP — run build.bat first"; exit 1; }
cp -f "$SRC_DSK" "$DSK" || exit 1
rm -f build/erase_trace.log
...
echo "[erase_trace] pri_slot $P_PRI_SLOT  ch_last $P_LAST  ch_move $P_MOVE"
...
"$MAME" coco3 \
    ...
    -seconds_to_run 60 \
    -autoboot_script harness/tools/erase_trace.lua \
    >/dev/null 2>&1

if [ -f build/erase_trace.log ]; then sed 's/^/  /' build/erase_trace.log
