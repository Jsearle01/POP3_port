-- harness/smoke/compiled_test.lua  (P1.3; adapted from cel_test.lua)
--
...
local BIN       = os.getenv("P_BIN")    or "build/compiled_probe.bin"
local EXPECT    = os.getenv("P_EXPECT") or "build/compiled_expect.txt"
local OUT       = os.getenv("P_OUT")    or "build/compiled_test.log"
local PASS_PATH = os.getenv("P_PASS")   or "build/compiled_test_PASS"
local FAIL_PATH = os.getenv("P_FAIL")   or "build/compiled_test_FAIL"

local ADDR_STATUS, ADDR_MAGIC = 0x0203, 0x0204
local MAGIC   = 0xC0DE
local FB_A    = 0x8000
local STRIDE  = 80
...
log("# cel_test.lua — POP P1.3 COMPILED-sprite verification")
...
local state = "boot"
_G._compiled_notifier = emu.add_machine_frame_notifier(function()
    ...
    if state == "verify" then
        ...
        check("framebuffer_matches_cel", bad == 0,
        ...
        log("# MAME snapshot written (see the snap/ dir) � surfaced for Jay, uninspected")
