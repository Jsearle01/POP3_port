...
def song_at(bi):
    """The song this beat STARTS, or 0. Read off the same PLAN row the beat came from,
    so the schedule cannot disagree with the plan about which beat is a cue.

    ★★ THE PLAN HAS ALWAYS KNOWN THIS. A `("song", "s_Princess", 761)` row already became
    a hold of its own measured length, and the name already appeared in cel_plan.s's
    COMMENT — but only in the comment, so nothing could act on it. P4.23 found the cutscene
    had no way to play a song at all; this is the column that gives it one.
    """
    w, seq, _n = PLAN[bi]
    if w != "song":
        return 0
    if seq not in SONG_ID:
        raise SystemExit("[bake_scene] PLAN names a song the oracle does not: %r" % seq)
    return SONG_ID[seq]
...
def emit_plan_s(p):
    ...
    because the symptom is not a missing cel but the wrong bytes drawn confidently.
        ...
        fcb  scenery    which scenery this beat shows (P3.85)
        fcb  song       the song this beat STARTS, oracle id, or 0 for none (P4.23)
    """
    ...
    for s in p["schedule"]:
        ...
        L.append("                fcb     %d,%d,%d           ; beat %-2d %-12s plays %-4d%s%s%s"
