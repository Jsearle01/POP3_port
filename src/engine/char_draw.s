...
* fails a minute into the first workload.
                import  glass_cels          ; the hourglass, in flame_cels.s
                import  flow_cels
* THE UNCLIPPED ENTRIES, and the hourglass is entitled to them where a character is not.
* blit_cel/blit_save/blit_erase honour bc_lead/bc_keep so a character can walk off the
* edge; the glass is scenery at a FIXED column well inside the visible window and never
* moves, so there is no window for it to be trimmed against. Going through the clipped
* path would mean setting bc_lead/bc_keep for it and restoring them for the characters —
* a second writer of the clip state, which is how the torches got dragged into P3.83's
* `leax a,x` signed-offset bug.
...
cf_bank_ok
* THE SCENERY GOES DOWN BEFORE THE CHARACTERS' SAVE, NOT MERELY BEFORE THEIR DRAW
* (P3.85). vm_frameadv's first pass captures the background each character will later
* restore; if the hourglass were drawn after it, the character would restore ROOM over
* glass and eat a hole in it, and if it were drawn between the save and the draw the
* saved background would be a frame stale. This is the same relationship the torches
* already have with the characters, one level up in room_loop.
                jsr     scenery_frame
                jsr     vm_frameadv             ; draw
                rts

* ---------------------------------------------------------------
* scenery_frame — the hourglass and its running sand. [SUBS.S:722-745 addglass1]
*
* WHAT THE ORACLE DOES, and it is only three stores: `ldx #0 / jsr addglass1` puts the
* glass up at the hourglass beat, `sta psandcount` starts the sand, and `ldx #1 /
* jsr addglass1` swaps the body to state 1 after Vexit. It never takes either down.
*
* ★ WHICH BEATS THOSE ARE IS A PLAN COLUMN, NOT A TEST IN HERE. vm_scenery is byte 5 of
* the beat row, generated by bake_scene.py from the same walk that fixes the plays, the
* block and the signature. Deciding it here would be a SECOND derivation of "which beat
* is it", and P3.78's beat-switch was exactly that failure — a mapping correct for the
* beat, applied to a cel from the one before.
*
* THE BODY IS REDRAWN EVERY FRAME AND HAS NO PEEL. It never moves and it is opaque
* [GAMEBG.S DRAWGLASS sets OPACITY = sta], so there is nothing to restore behind it and
* nothing that can accumulate. The SAND does have one, per buffer, because it cycles:
* the glass body's stream carries SKIP runs, so redrawing the body does NOT reliably
* cover last frame's sand, and assuming it would is how a smear gets shipped.
scenery_frame
                lda     vm_scenery
                bne     sc_on
...
* S INTO THE FRAMEBUFFER for the duration of a blast and restores it from bc_saved_s on
* the way out; anything held on the stack across the call is safe only for as long as
* that restore is correct. P3.81-83 spent three dispatches on two wrong flame bytes that
* came from a bsr executed while S pointed into the framebuffer. Nothing here needs the
* stack, so nothing here uses it.
                sta     sc_flags
* the sand's peel slot for the buffer being drawn, exactly as slot_peel does upstairs
                ldy     #flow_peel
                lda     ch_slot
                beq     sc_slot0
                leay    FLOW_PEEL,y
...
* checker; the checker was right.
                ...
                bita    #SC_GLASS1
                beq     sc_g0
                leau    2,u                     ; state 1, after Vexit [SUBS.S:745]
sc_g0
                ldu     ,u                      ; -> the segment stream
                ldx     ch_base
                leax    GLASS_OFF,x
...
* --- the lightning flash ---
* `lda #5 / sta lightning` then `lda #$ff / sta lightcolor` [SUBS.S:723-726], and the
* playback loop brackets each play with flashon/flashoff [SUBS.S:857-867]. lightcolor
* $FF is Apple hi-res WHITE, so it is a full-screen white strobe, five of them, one per
* play — NOT a five-frame flash, which is what the source line reads like until the
* trace shows `lightning` stepping 5,4,3,2,1,0 at eight-frame spacing.
*
* ★ IT IS A PALETTE WRITE, NOT A FILL, and that is the whole reason it is cheap enough
* to have. The Apple has to paint a screen to change its colour; the GIME has four
* palette registers and the framebuffer means whatever they say it means, so the flash
* costs four stores and no pixels — and, unlike a fill, it cannot damage what is under
...
sc_pal_normal
                lda     sc_wasl
                beq     sc_sand                 ; not lit last frame either — leave the
                clr     sc_wasl                 ;   palette alone, so the 16-colour swap
                ldx     #$FFB0                  ;   has one writer and not two
                ldu     #sc_pal
                lda     ,u+
                sta     ,x
                lda     ,u+
...
*                                               ;   [P3.91]; once per play when fixed
                ...
                lda     #FLOW_ROWS
                ldb     #FLOW_WIDE
                jsr     blit_save_full          ; capture body-over-room, not room
                ldx     sc_pslot
                lda     #1
                sta     ,x                      ; this buffer now has sand to erase
                ldx     ch_base
                leax    FLOW_OFF,x
                ldu     sc_data
                jsr     blit_cel_full
...
vm_scenery      fcb     0               ; byte 5 of the beat row — the hourglass flags

* ── the hourglass (P3.85) ───────────────────────────────────────────────────────────
* PLACEMENT IS DERIVED FROM THE ORACLE'S OWN CONSTANTS, and the mapping is the torches'
* [cutscene_room.s TORCH0_COL], not a scaling. The port centres the Apple's 280 px in
* 320, so CoCo px = Apple px + 20 — which is what puts ptorchx 13 (Apple px 91) at px
* 111 = byte 27 phase 3, and ptorchx 25 (px 181) at px 201 = byte 50 phase 1. Rows are
* 1:1, and the oracle's y is the BASELINE, so the top row is y - rows + 1.
*
*   glassx 19 -> Apple px 133 -> CoCo px 153 = byte 38 phase 1   [GAMEBG.S:120]
*   glassy 151 baseline, 25 rows            -> top row 127
*   flowx  20 -> Apple px 140 -> CoCo px 160 = byte 40 phase 0
*   flowy  149 baseline, 9 rows             -> top row 141
*
...
SC_GLASS1       equ     $04
SC_FLASH        equ     $08
FB_STRIDE_SC    equ     80              ; 320 px at 4 px/byte
GLASS_COL       equ     38
GLASS_TOP       equ     127
FLOW_COL        equ     40
FLOW_TOP        equ     141
FLOW_ROWS       equ     9
FLOW_WIDE       equ     2
GLASS_OFF       equ     GLASS_TOP*FB_STRIDE_SC+GLASS_COL
FLOW_OFF        equ     FLOW_TOP*FB_STRIDE_SC+FLOW_COL
FLOW_PEEL       equ     FLOW_ROWS*FLOW_WIDE
sc_flags        fcb     0               ; this frame's flags, off the stack on purpose
sc_flow         fcb     0               ; 0..2, the sand frame; psandcount's counterpart
sc_peel         fdb     0
sc_data         fdb     0
sc_pslot        fdb     0
...
cel_load_startup
                sty     cel_disk
* ★ THE DRIVE IS HELD ACROSS THE WHOLE SCHEDULE NOW (P3.84) and released the moment the
* last staged read completes, so dr_spinup's conditional skips every spin-up after the
* first. The HAL entry that releases it arrives in U the same way load_tracks arrives in
* Y — this bundle links without the HAL and every cross-image call is an argument.
                stu     cel_moff
                lda     #CEL_N_READS
                sta     cel_rd_left
...
cel_service_read
                ...
                bsr     cel_read_page
                bne     cr_failed
* ★ THE LAST ONE RELEASES THE DRIVE. Held across both reads it saves a spin-up on the
* second (~0.40 s, P3.76's A/B measurement); held past them it would spin for the rest of
* the scene for nothing. The count comes from the pack, so adding a read cannot leave the
* motor running.
                dec     cel_rd_left
                bne     cr_none
                jsr     [cel_moff]
                clra                            ; ...and the caller's Z means "read OK"
                rts
cr_failed
...
cel_moff        fdb     0               ; ...and its disk_read_motor_off
cel_rd_left     fcb     0               ; staged reads still owed; 0 releases the drive
