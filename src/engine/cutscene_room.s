...
* ★ FROM build.bat SINCE P5.16b — intro_seq.s caches this track and must agree with it.
* The local value survives as the standalone default so this file still assembles alone.
...
*
* ★★★ IT TAKES ONE ARGUMENT SINCE P5.16: Y = THE READER. The two fetches below used to be
* `jsr load_tracks` — this image's own copy — and that is why they were disk reads even
* after the intro had already cached every other asset it needs. The intro cannot reach in
* and change them, and this bundle cannot see the intro's cache, so the reader arrives as
* an argument, exactly as the disk already arrives as an argument for the cels
* (`ldy #load_tracks` at room_load_cels: "the bundle has no HAL and no room; the disk
* arrives as arguments"). The intro passes tc_fetch, which answers from RAM and has
* load_tracks' contract and clobber set; a standalone caller passes load_tracks, or passes
* nothing and gets it by default. Same code, same two steps, no disk.
*
* ★★ IT CHECKS NOTHING BACK, BY DESIGN. A failure here leaves rs_pre_ok unset and
* room_start does the reads itself exactly as it always did. The degradation is the old
* behaviour, so the caller needs no error path and this needs no contract beyond "try".
*
...
* ---------------------------------------------------------------
room_preloaded
* ★ Y = the reader, and ZERO means "the one in this image". STY sets Z from what it stored,
* so the default costs one branch — and it matters: a caller that forgets leaves this
* jumping through a null vector, which is the one failure this routine must not have.
                sty     rp_fetch
                bne     rp_havef
                ldy     #load_tracks
                sty     rp_fetch
...
rp_fetch        fdb     0               ; the caller's reader — see the entry above
