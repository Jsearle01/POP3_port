...
**Status: COMPLETE. 25.3 PASSED (Jay, live-disk, RGB, 128 KB).** This report spans four commits —
`1bd3948` (the work), `e7f64e7` (§11's hash), `ec397c3` (the live-runner fix, made while Jay was
watching), `bc179df` (the gate result). §11 lists them.
...
**★ ONE TRACKED FILE IS DIRTY AND IT IS NOT MINE TO CLEAN: `dist/mame-cfg/rgb/coco3.cfg`.** It was
already modified at receipt, and every gate run in this task modified it further, because
`-cfg_directory dist/mame-cfg/rgb` makes MAME **write the directory back on exit**. The file's own
header predicted this — *"MAME REWRITES this file on exit... it means the file is a template, not a
constant"* — but the rewrite **strips that header**, so the documentation of why the file exists is
destroyed by the runs that depend on it, and the file now carries an absolute
`C:\Projects\POP3_port\build\` path from this machine.

The gate itself is unaffected: `<port tag=":screen_config" ... value="1" />` survives the rewrite, so
RGB really was in force. **I have not reverted it** — it was dirty before this task and restoring it
would discard whatever a prior session left there, which is a ruling rather than a tidy-up. §7.7 and
§8.6 carry it.
...
- `harness/smoke/tile_live.lua`, `harness/smoke/run_tile_live.sh` — the throttled, windowed gate
  runner for Jay (RGB, `-ext fdc`, `-flop1`, never exits). **Amended in `ec397c3`** after its first
  launch: the probe block is now WATCHED for transitions rather than sampled once at a guessed
  delay. See 25.3 in §5.
...
5. **`tile_live.lua` was amended AFTER the first report commit** (`ec397c3`), in response to Jay
   asking me to run the gate. This is a fix to a harness defect the run exposed, not a change to
   anything under gate — the port binary and the disk image are untouched by it, and the framebuffer
   Jay saw is the same one the suite proves byte-exact. Recorded because a report that describes a
   file and a tree that has since changed it is the exact mismatch §7's route-accounting rule exists
   to catch.
...
**Nothing else was proposed and dropped.** The five follow-ups in §8 are surfaced as next tasks, not
described as work this commit contains.
...
7. **★ EVERY GATE RUN REWRITES A TRACKED FILE.** `-cfg_directory dist/mame-cfg/rgb` makes MAME write
   that directory back on exit, so the RGB template is modified by the runs that depend on it — and
   the rewrite **strips the header comment that explains why the file exists**, replacing it with
   `<!-- This file is autogenerated; comments and unknown tags will be stripped -->` plus an absolute
   path from whichever machine ran last. `screen_config value="1"` survives, so no gate has been run
   in the wrong mode; what is being lost is the documentation, one run at a time. Left dirty rather
   than reverted (§0). Ten-plus runners take this path, so this is not specific to the tile gate.
...
6. **★ Copy the RGB template to a SCRATCH cfg directory before every run** (flag 7). The runners
   already do exactly this for the disk image — `cp -f build/probe.dmk build/run_*.dmk`, because MAME
   opens a floppy read-write and saves back. The cfg directory is the same hazard and got the
   opposite treatment, and the file's own header has been warning about it the whole time. One-line
   change per runner, or one shared line in `ramsize.sh`-style; it stops the template eroding and
   makes `git status` mean something again after a gate.
...
Four commits on `origin/wip`, oldest first:

| commit | what |
|---|---|
| `1bd3948` | the work — bake, renderer, link scripts, tools, harness, build integration, this report |
| `e7f64e7` | §11's own hash, recorded after the push |
| `ec397c3` | the live gate watches the status byte instead of sampling it (§6.5) |
| `bc179df` | 25.3 PASSED — Jay, live-disk, RGB, 128 KB (§5) |

Plus this update. **`main` is untouched at `32b5fe2`** — no dispatch authorised moving it.

Pool: `seeds/POP/live/2026-08-21-the-capacity-check-that-passed-was-not-the-one-that-binds.md`,
pushed as `9445650` in the candidate-pool repo.
